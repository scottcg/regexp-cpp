...
namespace re {
	...
	int re_engine<syntaxType>::exec_optimize() {
		...
		if (continue_work == false || char_count < 2) {
			// don't bother if just a single char
			return 0;
		}

		int old_cursor = 0;
		if (code[old_cursor] != OP_END) {
			// just a plain string
	...
	inline ostream &operator <<(ostream &out, const runtime_backref &p) {
