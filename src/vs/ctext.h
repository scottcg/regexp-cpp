...
namespace  re {
	...
	template <class traitsType> class re_ctext {
	public:
		...
		explicit operator char_type () const {	// vc5 must have this inline
			if ( m_text_end == m_str1_end && m_text < m_str1 ) return 0;
			if ( m_text == m_text_end ) return 0;
			assert(m_text != 0);
			return *m_text;
	...
	template <class traitsType>typename re_ctext<traitsType>::char_type re_ctext<traitsType>::operator ++ (int) {
	...
	int re_ctext<traitsType>::position() const {
		if ( m_text_end == m_str2_end ) {	// we are on the second string
